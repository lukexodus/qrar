    ...
    try:
        ...
        while True:
            try:
                ...
                break
            except (ConnectionAbortedError, httplib2.ServerNotFoundError, httplib2.error.ServerNotFoundError, TransportError, SSLEOFError, SSLError):
...
# Retrives the backup 'attendance', 'streak', and 'namesRowPositions' dictionaries, otherwise initializes it
with shelve.open('backup') as backup:
    ...
    try:
        ...
        logging.info(f"previousActiveCellsText: List[list] = backup['previousActiveCellsText'] = {backup['previousActiveCellsText']}")
    except KeyError:
...
if not noInitialFile:
    wb = openpyxl.load_workbook(excelFilename)
    ws = wb.active
    activeCellsObjects = \
        ws['A1':f'{get_column_letter(ws.max_column)}{ws.max_row}']
    activeCellsText: List[list] = \
        [[cell.value for cell in row] for row in activeCellsObjects]
    logging.info(f"activeCellsText: List[list] = [[cell.value for cell in row] for row in activeCellsObjects] = {activeCellsText}")
    onlineDatabaseCompatibleActiveCellsText = \
        [[' ' if cell.value == None else cell.value for cell in row] for row in activeCellsObjects]
    if activeCellsText != previousActiveCellsText:
        ...
        if (len(activeCellsText) != len(previousActiveCellsText)) or \
            ...
            newPreviousActiveCellsTextTemplate = \
                [['' for i in range(len(activeCellsText[0]))] for i in range(len(activeCellsText))]
        ...
        for rowNum in range(1, ws.max_row + 1):
            for colNum in range(1, ws.max_column + 1):
                ...
                onlineDatabaseCompatibleCurrentCellText = onlineDatabaseCompatibleActiveCellsText[rowNum - 1][colNum - 1]
                if (len(activeCellsText) != len(previousActiveCellsText)) or \
                    (len(activeCellsText[0]) != len(previousActiveCellsText[0])):
                    ...
                    def writeData(sheet1, name, date, initialDate, clockTime):
                        while True:
                            try:
                                ...
                                httplib2.error.ServerNotFoundError, TransportError, SSLEOFError, SSLError, AttributeError):
                                print('Connection error. Please check your internet connection. ', end='', flush=True)
                                countdown()
                                continue
                    ...
                    def sendEmail(name, email, date, clockTime, currentStreak, dailyQuote):
                        ...
                        while True:
                            try:
                                ...
                                print(f'Error sending email to {email}. Please check your internet connection. ', end='', flush=True)
                                countdown()
                                continue
                    
                    threading.Thread(target=sendEmail, args=[name, email, date, clockTime, currentStreak, dailyQuote]).start()
...
# (When the program is ended)
    ...
    while True:
        ...
        try:
            ...
            logging.info(f"activeCellsText: List[list] = [[cell.value for cell in row] for row in activeCellsObjects]\nbackup['previousActiveCellsText'] = activeCellsText = {activeCellsText}")
            wb.save(excelFilename)
            ...
            if useOnlineDatabase and not uploadedToGoogleSheets:
                while True:
                    try:
                        ...
                        with open('log.txt', 'a', encoding='utf-8') as log:
                            log.write(traceback.format_exc())
                        print('Upload to Google Sheets failed. Please check your internet connection. ', end='', flush=True)
                        countdown(5)
                        continue
                        ...
                        while True:
                            try:
                                ...
                                with open('log.txt', 'a', encoding='utf-8') as log:
                                    log.write(traceback.format_exc())
                                print('Upload to online database failed. Please check your internet connection. ', end='', flush=True)
                                countdown()
                                continue
                    else:
                        with shelve.open('googleSheetsConfig') as googleSheetsConfig:
